...
###   hiphopcoincore.org and hiphopcoin.org and compares them.
...
WORKINGDIR="/tmp/hiphopcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://hiphopcoincore.org"
HOST2="https://hiphopcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="hiphopcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hiphopcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "hiphopcoin.org failed to provide signature file, but hiphopcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "hiphopcoin.org and hiphopcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Hiphopcoin Core binary release signing key installed?"
   fi
