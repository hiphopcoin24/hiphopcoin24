...
const char * const HIPHOPCOIN_CONF_FILENAME = "hiphopcoin.conf";
const char * const HIPHOPCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/hiphopcoin/hiphopcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hiphopcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", HIPHOPCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "hiphopcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Hiphopcoin
    // macOS: ~/Library/Application Support/Hiphopcoin
    // Unix-like: ~/.hiphopcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hiphopcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Hiphopcoin";
#else
    // Unix-like
    return pathRet / ".hiphopcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", HIPHOPCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Hiphopcoin Core copyright is not removed by accident
    if (copyright_devs.find("Hiphopcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Hiphopcoin Core developers";
    }
