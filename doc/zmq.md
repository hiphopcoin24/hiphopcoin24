...
The Hiphopcoin Core daemon can be configured to act as a trusted "border
router", implementing the hiphopcoin wire protocol and relay, making
...
The ZeroMQ feature in Hiphopcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building hiphopcoind:
...
For instance:
    ...
    $ hiphopcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/hiphopcoind.tx.raw \
...
These options can also be provided in hiphopcoin.conf.
...
From the perspective of hiphopcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into hiphopcoind directly. Furthermore, no information is
...
using. Hiphopcoind appends an up-counting sequence number to each
